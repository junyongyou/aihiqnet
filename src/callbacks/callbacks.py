...
def create_callbacks(model_name, result_folder, other_callback=None, checkpoint=True, early_stop=True, metrics='accuracy', info='base'):
    ...
    csv_logger.set_base_fine(info)
    callbacks.append(csv_logger)
    if early_stop:
        callbacks.append(EarlyStopping(monitor='plcc', min_delta=0.001, patience=30, mode='max'))
    if checkpoint:
        if metrics == None:
            # mcp_file = os.path.join(result_folder, '{epoch:01d}_{loss:.4f}_{val_loss:.4f}.h5')
            mcp_file = os.path.join(result_folder, '{epoch:01d}_{loss:.4f}.h5')
        else:
            if metrics == 'accuracy':
                ...
                # mcp_file = os.path.join(result_folder, '{epoch-1:01d}_{loss:.4f}_{val_loss:.4f}.h5')
                mcp_file = os.path.join(result_folder, '{epoch:01d}_{loss:.4f}.h5')
