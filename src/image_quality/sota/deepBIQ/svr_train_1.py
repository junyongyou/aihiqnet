...
def main():
    ...
    with (open(r'..\databases\results\deepBIQ\scaler_koniq_small.pkl', 'wb')) as scaler_file:
        dump(scaler_x, scaler_file)
    # scaler_x = load(open(r'..\databases\results\deepBIQ\scaler.obj', 'rb'))
    joblib.dump(scaler_x, r'..\databases\results\deepBIQ\scaler_koniq_small_1.pkl')
    ...
    clf.fit(X_train, y_train)
    with open(r'..\databases\results\deepBIQ\biq_model_koniq_small.pkl', 'wb') as clf_file:
        dump(clf, clf_file)
    joblib.dump(clf, r'..\databases\results\deepBIQ\biq_model_koniq_small_1.pkl')
...
def validation():
    ...
    X_test, y_test = collect_features_scores(test_image_files, test_scores)
    with (open(r'..\databases\results\deepBIQ\scaler_all.obj', 'rb')) as scaler_file:
    ...
    #     clf = load(cf)
    clf = joblib.load(r'..\databases\results\deepBIQ\biq_model_all_1.obj')
    pred_y_test = clf.predict(X_test)
...
if __name__ == '__main__':
    ...
    # with open(r'..\databases\results\deepBIQ\biq_model_normal_1.obj', 'rb') as cf:
    #     clf = load(cf)
