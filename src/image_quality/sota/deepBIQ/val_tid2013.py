...
def cal_features(image_files):
    ...
    for i, train_image_file in enumerate(image_files):
        ...
        feature_path = train_image_file.replace(r'F:\tid2013\distorted_images',
                                                r'..\databases\AlexNet_features\tid2013').replace(
            extname, '.npy')
...
def generate_data():
    val_folders = [r'F:\tid2013\distorted_images']
    spaq_mos_file = r'F:\tid2013\mos_with_names.txt'
    image_scores = get_image_scores_tid2013(spaq_mos_file)
...
def collect_features_scores(image_files, scores):
    ...
    for image_file, score in zip(image_files, scores):
        ...
        feature_path = image_file.replace(r'F:\tid2013\distorted_images',
                                                r'..\databases\AlexNet_features\tid2013').replace(
            extname, '.npy')
...
def get_image_scores_tid2013(mos_file):
    image_files = {}
    with open(mos_file, 'r+') as f:
        lines = f.readlines()
        for line in lines:
            content = line.split(' ')
            image_file = content[1].strip()
            score = float(content[0]) / 2.5 + 1
