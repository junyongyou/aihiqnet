...
from tensorflow.keras.layers import Lambda, Conv2D, LSTM, Concatenate, GlobalMaxPooling2D, GlobalAveragePooling2D

from tensorflow.keras.layers import Dropout, Activation, Dense, Reshape, Flatten
from tensorflow.keras.layers import Input, Add, multiply, Permute
...
def identity_block(input_tensor, kernel_size, filters, stage, block, use_bias=True, dilation_rate=(1, 1),
                   train_bn=None):
...
def conv_block(input_tensor, kernel_size, filters, stage, block, use_bias=True, strides=(2, 2), dilation_rate=(1, 1),
               train_bn=None):
...
def kerasResnet(input_tensor=None, train_bn=None, weights=None):
...
def M_VGG16(input_tensor=None):
    ...
    # conv_5
    conv5_1 = Convolution2D(512, kernel_size=(3, 3), activation='relu', padding='same', dilation_rate=(1, 1))(
        conv4_pool)
...
def SGDNet(basemodel='resnet', saliency='output', CA=True, img_rows=480, img_cols=640, fixed=False, out1dim=512,
           out2dim=512, weights=None, train_bn=None):
    inputimage = Input(shape=(img_rows, img_cols, 3))

    #    input_ml_net2 = Input(shape=(int(img_rows/8), int(img_cols/8),1))
    if basemodel == 'resnet':
        base_model = kerasResnet(input_tensor=inputimage, train_bn=None, weights=weights)
    else:
    ...
    if saliency == 'output':
        sm = Convolution2D(1, kernel_size=(1, 1), activation='relu', padding='same', use_bias=False, name='saliency')(
            fraw)
