...
from image_quality.sota.sdgnet_original.utilitiestrain import preprocess_imagesandsaliencyforiqa, preprocess_label
import time

from image_quality.sota.sdgnet_original.modelfinal import TVdist, SGDNet
...
def datasetgenerator(conf, EXP_ID='0', mostype='ss'):
    # im_dir = conf['im_dir']
...
class DataGenerator(Sequence):
    ...
    def __data_generation(self, list_IDs_temp):
        ...
        X, X2 = preprocess_imagesandsaliencyforiqa(images[0:len(index)], simages[0:len(index)], shape_r, shape_c,
                                                   mirror=self.mirror, crop_h=shape_r, crop_w=shape_c)
...
if __name__ == '__main__':
    ...
    # print (crop_h) #384
    crop_w = 512
    model = SGDNet(basemodel=args.basemodel, saliency=args.saliency, CA=args.CA, fixed=args.fixed, img_cols=crop_w,
                   img_rows=crop_h, out2dim=args.out2dim)
    ...
    model.summary()
    train_index, val_index, test_index = datasetgenerator(config, args.exp_id, args.mostype)
    ...
    if args.phase == 'train':
        ...
        checkpointdir = '../checkpoint/' + 'saliency{}-alpha{}-{}-{}-{}-EXP{}-lr={}-bs={}'.format(args.saliency,
                                                                                                  str(args.alpha),
                                                                                                  args.mostype,
                                                                                                  args.database,
                                                                                                  str(args.out2dim),
        ...
        train_generator = DataGenerator(train_index, config, b_s, shuffle=True, mirror=False, mostype=args.mostype,
                                        saliency=args.saliency)
        val_generator = DataGenerator(val_index, config, 1, shuffle=False, mirror=False, mostype=args.mostype,
                                      saliency=args.saliency)
        if args.saliency == 'output':
            model.fit_generator(generator=train_generator, epochs=args.epochs,
                                steps_per_epoch=int(1 * nb_imgs_train // b_s),
                                validation_data=val_generator, validation_steps=int(1 * nb_imgs_val // 1),
                                callbacks=[EarlyStopping(patience=50),
                                           ModelCheckpoint(
                                               checkpointdir + '.{epoch:02d}-{val_loss:.4f}-{loss:.4f}-{val_predictions_loss:.4f}-{val_saliency_loss:.4f}.pkl',
                                               save_best_only=True),
                                           reduce_lr])
        else:
            model.fit_generator(generator=train_generator, epochs=args.epochs,
        ...
        for i in range(repeat):
            ...
            test_generator = DataGenerator(test_index, config, 1, shuffle=False, mirror=False, mostype=args.mostype,
                                           saliency=args.saliency)
