...
def phiq_subnet(n_quality_levels, input_shape=(None, None, 3), naive_backbone=False, backbone='resnet50',
                fpn_type='fpn', attention_module=True, pooling='max', return_backbone_maps=False,
...
def evaluation__scales(model, image_files, scores, imagenet_pretrain=True):
    predictions_all = []
    mos_scores = []
...
def generate_activations(image_file, pooling, activation_model=None):
    if activation_model is None:
        activation_model = phiq_subnet(n_quality_levels=5,
                                       ...
                                       fpn_type='fpn',
                                       pooling=pooling)
        weights = r'..\databases\experiments\koniq_normal\resnet50_distribution_attention_fpn_finetune\09_0.8493_0.9294.h5'
...
def correlation_scales():
    ...
    evaluation__scales(model, image_files, scores, imagenet_pretrain=True)
...
def generate_backbone_maps(image_file, pooling, original_model, feature_model):
    ...
    # feature_model_1 = phiq_subnet(n_quality_levels=5, pooling=pooling, return_backbone_maps=True)
    # feature_model_1.load_weights(weights, by_name=True)
...
def cal_features(image_file):
    feature_model = phiq_subnet(n_quality_levels=5, return_features=True)
    weights = r'..\databases\experiments\koniq_normal\resnet50_distribution_attention_fpn_finetune\09_0.8493_0.9294.h5'
    image = np.asarray(Image.open(image_file), dtype=np.float32)
    image /= 127.5
    image -= 1.
...
if __name__ == '__main__':
    ...
    # image_file = r'..\databases\train\koniq_normal\5354698.jpg'
    # image_file = r'..\databases\train\koniq_normal\125829275.jpg'
    # image_file = r'..\databases\train\koniq_normal\414892775.jpg'
    # image_file = r'..\databases\train\koniq_normal\194278089.jpg'
    # pooling = 'max'
    pooling = 'max'
